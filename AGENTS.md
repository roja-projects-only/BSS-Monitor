...
### Modularization & Organization Practices

This project is **client-side only** — it runs via a Roblox script executor (Delta, Seliware, Synapse, etc.) injected into the client. It has no server authority. All actions (kicks, bans) work by simulating chat commands as the local admin player. Modules cannot use `require()` — every dependency is loaded via `loadstring` and injected through `Init()`.

#### Module Responsibilities (Single Responsibility)
Each module owns exactly one concern. Do not mix concerns across files:
| Module | Owns |
|---|---|
| `config.lua` | All configuration, whitelist logic, constants |
| `logger.lua` | Log buffering, console filtering, listener callbacks |
| `scanner.lua` | Hive/bee data extraction from the game world |
| `chat.lua` | Platform detection, VirtualInputManager, chat input |
| `monitor/state.lua` | Shared runtime state tables only — no logic |
| `monitor/ban.lua` | Ban execution, verification, retry logic |
| `monitor/cycle.lua` | Scan loop timing, timeout kicks, start/stop |
| `monitor/init.lua` | Monitor orchestrator — wires sub-modules, exposes unified API |
| `webhook/http.lua` | HTTP abstraction, executor fallback chain |
| `webhook/embeds.lua` | Embed builders for each notification type |
| `webhook/init.lua` | Webhook orchestrator — unified send API |
| `gui/theme.lua` | Color palette and size constants only |
| `gui/helpers.lua` | Low-level UI primitives (no business logic) |
| `gui/components.lua` | Composite UI component builders |
| `gui/init.lua` | GUI orchestrator — lifecycle, layout, live refresh |

#### Sub-module Grouping
Logical sub-systems are grouped in subfolders (`monitor/`, `webhook/`, `gui/`). Each group follows the same pattern:
- **Leaf files** (`state.lua`, `ban.lua`, `http.lua`, `theme.lua`, etc.) — focused, no knowledge of sibling modules
- **`init.lua` orchestrator** — imports and wires all siblings, exposes the group's unified public API

When a new sub-system is needed, create a new subfolder with the same pattern. Do not dump new files into `modules/` root unless they are standalone with no siblings.

#### Dependency Injection via Init()
Modules **never** reference globals or siblings directly. All dependencies are passed in through `Init()`:
```lua
-- Good: dependency injected
function MyModule.Init(Config, Logger)
    _Config = Config
    _Logger = Logger
end
...
function MyModule.DoThing()
    print(_G.BSSMonitor.Config.SOME_VALUE)  -- never do this inside a module
end
```
The only global used by modules is `_G.BSSMonitor`, and only as the **final export** set by `loader.lua`/`main.lua` — never read back internally.

#### When to Create a New Module
- New concern that doesn't belong in any existing module → new file
- An existing file exceeds ~300 lines and has separable sub-concerns → split it
- New sub-system with 2+ related files → new subfolder with `init.lua` orchestrator

#### When NOT to Create a New Module
- A single helper function needed once → add it to the most relevant existing file
- Config-only additions → always go in `config.lua`, never a new file
- State-only additions → always go in `monitor/state.lua`

#### Naming Conventions
- **Files**: `lowercase_snake_case.lua`
- **Module tables**: `PascalCase` (e.g. `MonitorBan`, `WebhookHttp`, `GUITheme`)
- **Private module-locals**: `_PascalCase` prefix (e.g. `_Config`, `_Logger`)
- **Public functions**: `Module.VerbNoun()` (e.g. `Ban.ExecuteWithVerification()`, `Chat.SendKickCommand()`)
- **Constants in Config**: `UPPER_SNAKE_CASE`
- **State table keys**: `PascalCase` (e.g. `State.IsRunning`, `State.BannedPlayers`)

#### File Size & Complexity Guidelines
- Keep modules under ~300 lines where possible
- If a module needs more than ~5 injected dependencies, consider whether it has too many responsibilities
- `init.lua` orchestrators may be longer since their job is wiring — but they should contain **no business logic**
...
- **Commit freely without pulling first.** `git commit` is local and never requires a pull. **Do not pull or push** — the user will handle syncing with the remote manually. Only commit locally after each task.
